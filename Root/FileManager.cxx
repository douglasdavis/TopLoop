...
void TL::FileManager::enableParticleLevel() { m_doParticleLevel = true; }

void TL::FileManager::setTreeName(const std::string& tn) { m_treeName = tn; }

void TL::FileManager::setWeightsTreeName(const std::string& tn) { m_weightsTreeName = tn; }

void TL::FileManager::setParticleLevelTreeName(const std::string& tn) { m_plTreeName = tn; }

void TL::FileManager::setTruthTreeName(const std::string& tn) { m_truthTreeName = tn; }
...
void TL::FileManager::feedDir(const std::string& dirpath,
  ...
  if (boost::algorithm::ends_with(p.string(), "root") && need_symlink) {
    ...
    std::string symlink_name =
        abs_parent.string() + "/.TL_FileManager_symlinks/" + lose_root;
  ...
  for (const auto& i : fs::directory_iterator(loop_over)) {
    if (!fs::is_directory(i.path())) {
      ...
      boost::algorithm::split(dupeSplits, i.path().filename().string(),
                              boost::is_any_of("."));
  ...
  if (!isaf2) {
    for (const auto& si : sis) {
      if (si.dsid == m_dsid && m_campaign == si.campaign) {
        logger()->info("DSID {} for campaign {} is in the shuffle list", si.dsid,
  ...
  if (std::unique(std::begin(m_fileNames), std::end(m_fileNames)) !=
      std::end(m_fileNames)) {
    logger()->error("You have duplicate files in your dataset!");
    return;
  }
