...
int main(int argc, char *argv[]) {
  ...
  //  ttZloop* ttza = new ttZloop();
  auto ttza = std::make_unique<ttZloop>();
  ttza->setOutfileName("ttZout.root");
  
  if ( vm.count("feed-dir") ) {
    ttza->fileManager()->feedDir(vm["feed-dir"].as<std::string>());
  }

  if ( vm.count("feed-dir-all") ) {
    ttza->fileManager()->feedDir(vm["feed-dir-all"].as<std::string>(),true);
  }
  
  if ( vm.count("feed-txt") ) {
    ttza->fileManager()->feedTxt(vm["feed-txt"].as<std::string>());
  }
