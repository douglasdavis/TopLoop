...
class SampleMetaSvc final : public TL::Loggable {
 private:
  ...
  std::map<TL::kSgTopNtup, std::map<TL::kCampaign, float>> m_campaignLumis;

  void setupMaps();

  typedef std::map<int, std::tuple<TL::kInitialState, TL::kGenerator, TL::kSampleType>>
      SampleTable_t;
  ...
  auto as_integer(const Enumeration value) const ->
